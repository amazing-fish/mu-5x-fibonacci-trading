...
日志先于对应健康状态提交。消费先读日志，再读健康并核对 cycle；未匹配时继续推进日志游标并保留提醒，但禁止入场发送，不用旧健康或旧扫描名单永久撤销新结果。健康追上后，即使没有新日志也会完成核对。时间顺序只在同一服务运行内比较；只有与日志匹配的当前健康 cycle 可以确认新运行并重新建立顺序，支持时钟回拨后的干净重启。回拨使旧提醒失去时效依据时会撤销旧提醒，再记录当前运行的新证据。
