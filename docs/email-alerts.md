...
通知流分别保存排序所用的运行身份与最新观测已确认的运行归属。新日志尚未匹配健康 cycle 时，观测归属为 unknown；健康追上后补齐，即使仍是同一运行、相同时点和结果也会确认。当前运行首轮失败即使遇到小幅时钟回拨，仍会撤销明确属于旧运行的证据；不会仅因沿用旧排序身份而误撤销等待健康确认的新日志。
