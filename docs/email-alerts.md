...
日志 writer 和入场发送者共享 `observations.jsonl.lock`。发送者从最终核验到 SMTP 返回一直持有发布锁，writer 无法在检查通过后插入新的已提交结果；writer 已持锁时，发送者不等待、不消耗次数，稍后重试。进程结束或中断会释放操作系统锁，不删除锁文件。该锁不覆盖行情刷新，但慢 SMTP 会推迟扫描日志发布；每项网络操作超时 20 秒，扫描仍受信号服务原有超时预算约束，发布延迟与超时需纳入连续运行验收。
