...
- Trusted publication, refresh-input snapshotting, and consumer snapshotting share one cross-process critical section keyed to the trusted store. On POSIX it locks the existing store directory; on Windows it uses a path-derived named mutex, so consumers create no lease or pin file. A writer holds the lock across generation-manifest write, `current.json` replacement, and reclamation. Before any history/incremental request, a refresh holds it while resolving the latest current manifest and copying only the requested reusable dataset bytes; a consumer holds it across the same manifest-to-byte boundary. Automatic single-query loads snapshot only that query's effective dataset keys. Explicit `open_context(dataset_keys=...)` supports a multi-query key union, while omitting `dataset_keys` retains the full manifest dataset snapshot for universe-wide reuse. Network calls and later parsing/validation use those immutable bytes outside the lock, so reclamation cannot invalidate an in-flight refresh or opening context and cannot break a context that has already returned. Manifestless generation directories are treated as in-progress/crash remnants and are never reclaimed by the ordinary committed-generation policy; malformed or unreadable entries are reported and preserved without blocking independently validated old generations from reclamation.
