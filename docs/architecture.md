...
Candidate and approved-release publication use one strategy-domain durability boundary. A file-sync'd unique temporary is never authoritative. A canonical pending record binds the exact content digest and newly created parent lineage before an atomic hard-link, no-overwrite final-path install; a failed exclusive pending write removes only its identity-matched partial inode and syncs the parent before erroring. Recovery first re-syncs a complete record and its directory, then the recorded parent lineage, before touching the final path. After the final directory is durable, the publisher hard-links the durable record to a positive committed witness and syncs that witness entry; this is the artifact commit point. Pending remains a reader-priority barrier while the witness may be only transiently visible, closing the concurrent witness-visibility window. A failed post-commit barrier transition restores pending from the witness through link or exact-copy fallbacks; if no barrier can be recreated, a second directory fsync attempts to prove the deletion durable. A second fsync failure with pending still absent is a committed outcome rather than an error paired with consumable evidence: final and witness were already durable, and restart can only preserve that state or restore the matching recovery barrier. Readers and writers use `lstat` plus opened-file identity checks and accept sidecars only as stable regular files; comparison and fsync occur on that verified handle, including witness-install collisions. Symlinks, special files, or sidecar swaps therefore cannot masquerade as the no-sidecar legacy case or produce a reported-success artifact that the reader immediately rejects. Readers require an exact witness when publication sidecars exist, while retaining genuine no-sidecar legacy JSON compatibility. Identical final bytes are idempotent, different bytes conflict without overwrite even when created concurrently, and pre-commit failures or a visible pending barrier require exact-content recovery. The same-directory hard-link requirement fails closed on unsupported filesystems, with no replace fallback. This protocol is intentionally not shared with trusted-generation publication, Stage 0 append logs, or future execution ledgers, whose commit and recovery models remain separate.
...
- `trusted_run_id` is required and identifies the exact generation used to create the intent. `data_content_sha256` is a sorted interval/hash tuple covering the effective trusted data used by the scan, including the `5m` dependency. Missing hashes or a plain legacy bundle cannot create an intent. Neither field may be replaced in place when the current generation advances.
...
After creation, every field is immutable. Within one environment-specific business action, a change to price, size, leverage, decision, trusted-data reference, strategy/config, or expiry creates a new fingerprint, intent revision, and confirmation. An environment change is not an intent revision and may not supersede the existing intent: it creates a new environment-specific `business_action_id`, `audit_correlation_id`, authorization/confirmation lineage, audit stream, `mutation_action_id`, idempotency namespace/key, and `clOrdId`. A newer same-environment revision may explicitly supersede the older revision under that business action only while no broker-mutation reservation exists anywhere in the action, regardless of revision, and only by winning the same atomic action-selection fence used by first reservation. Once a leverage or submit mutation is reserved or observed, another revision cannot acquire a new submit key; replacement requires an explicitly designed cancel/replace transition. Creation time is retained from the first durable record only when a duplicate same-environment factory call finds the same full fingerprint.
...
| 30-B | Immutable `OrderIntent` and factory, Stage 1/3 | Add v1 signal-lineage/environment-specific-business-action identity, intent revision/fingerprint, exact allowlist, trusted/config binding, expiry, and dry-run review rendering. No adapter or confirmation. | 30-A. | Mutation matrix; same-environment full-fingerprint reuse; same-environment/same-action pre-reservation supersession; separate Demo/Production business action and audit identity for one signal; no cross-environment supersession/alias; trusted-generation rollover; post-reservation revision conflict; missing/legacy data rejection; `UNKNOWN` rejection; decimal canonicalization. | None. | Any unresolved lineage/action identity, field, generation, or expiry policy keeps the factory unavailable. |
