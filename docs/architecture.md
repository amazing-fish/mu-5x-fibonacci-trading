...
- The standalone refresh command enables bounded generation retention only after `current.json` publication. Completed generations are ordered by directory modification time with generation ID as a deterministic tie-breaker; it keeps the newest `--keep-generations N` directories (default `3`, minimum `1`), the current generation unconditionally even when current falls outside that window, and any generation carrying a strict self-bound `retention-pin.json` for strategy-release provenance. Malformed manifests or pins are preserved and reported. Every refresh use case uses an explicit context-managed cross-process lifecycle lock on the generations repository from prior-manifest selection through publication and reclamation, so direct and retention-enabled refreshes cannot delete another refresh's incremental-reuse input and every pre-publication exception releases the lock. Publication, reclamation, and load-context creation additionally share the shorter store critical section: POSIX locks the relevant directory and Windows uses canonical-path-derived `Global\\` named mutexes so service, scheduled-task, and interactive sessions serialize on the same store. Global mutexes use a protected explicit DACL granting access to SYSTEM, Administrators, and Authenticated Users rather than the creator account's default DACL. Explicit `open_context()` copies every manifest-bound CSV while holding the store lock. An automatic query copies only its effective dataset bytes and opens read-only leases for other manifest files; Windows handles opt into delete sharing and POSIX descriptors remain readable after unlink, so reclamation can proceed while later context extension still reads the pinned inode. Leases close with the context. Parsing and policy evaluation happen after lock release. Reclamation is attached to the canonical returned `RefreshRun`, the bounded run log, and command output rather than only at the CLI adapter. A post-replace pointer-directory sync warning skips deletion for that cycle. Each deletion is revalidated as a direct, non-symlink child of `generations/` using the canonical storage-segment validator. `--dry-run` publishes normally and reports protected IDs, candidates, and bytes without deleting.
