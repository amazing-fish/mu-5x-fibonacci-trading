...
- `LoadTrustedBundle.open_context()` remains read-only and snapshots the immutable bytes for every dataset named by the pinned manifest before returning. Later loads parse and validate those bytes, so a concurrent refresh may reclaim the old directory without breaking an already-open consumer operation; no consumer lease, pin file, or other storage mutation is required.
