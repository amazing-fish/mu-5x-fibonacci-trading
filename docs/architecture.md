...
Promotion verifies the trusted live PR author/commit lineage, review independence, and the GraphQL review node's unedited creation state before capturing approval evidence. Runtime resolution is deliberately offline: `StrictStrategyReleaseResolver` requires an exact content-addressed release ID plus expected rule and symbol, binds the config/dataset/supported symbol identities, and has no by-name/latest/current-pointer fallback. SCM establishes approval authenticity at promotion time; runtime checks the embedded snapshot and content integrity.
