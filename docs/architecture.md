...
- Trusted publication and consumer snapshotting share one cross-process critical section keyed to the trusted store. On POSIX it locks the existing store directory; on Windows it uses a path-derived named mutex, so consumers create no lease or pin file. A writer holds the lock across generation-manifest write, `current.json` replacement, and reclamation; a reader holds it across current-manifest resolution and immutable dataset-byte snapshotting. Later loads parse and validate those pinned bytes, so reclamation cannot race an opening context and cannot break a context that has already returned. Manifestless generation directories are treated as in-progress/crash remnants and are never reclaimed by the ordinary committed-generation policy.
