...
The observation-only email consumer is `mu_strategy.notifications`: it reads committed Stage 0 records and consistent service-health snapshots, persists event identities/cursors and delivery attempts in a separate SQLite outbox, and uses explicit NetEase SMTP configuration only with `email_alerts run --send`. It does not import a broker or re-evaluate strategy rules. Observation and health-state writers share the entry sender's publication fence from final validation through SMTP completion. `file_locks.py` owns the OS locking primitive also used by service-health instance locks; slow SMTP can delay logs, health checkpoints and subsequent scheduling within existing service timeout constraints. See [email reminders and forward-evidence preparation](email-alerts.md) for deadlines, unknown delivery outcomes, operator reconciliation and source-history boundaries.
...
| Typed decision | `EntryScanResult` and `ExecutionDecision` carry a typed `decision_code` and derive disposition/stage from the shared catalog. Dry-run uses one `scan_cycle.ScanCycle` with canonical provenance and non-`UNKNOWN` result validation, independent of optional Stage 0 persistence. | Versionless order planning still uses compatibility `action == "enter"`; confirmed Demo retains its legacy `UNKNOWN` compatibility. Typed metadata does not yet reach a confirmed order. |
...
The local review application stores manual fills separately: `manual_positions.py` owns manual confirmation, source binding, append-only fill revisions and the derived quantity/cost view in `positions.sqlite3` beside personal feedback. Independent state revisions capture manually confirmed stage/stop fields against the position's fill sequence and prior state revision under one SQLite transaction. Any new fill revision makes that confirmation require review; old ledgers remain unconfirmed without a read-time migration. `signal_review_server.py` handles local form transport, while `viz/position_ledger.py` renders both editors through one page shell and shares cards with live/static signal review. These manually recorded positions are not broker snapshots or complete `PositionStateSnapshot` inputs; management status remains unknown, and shared scan/position rules and position notifications remain unconnected. See [signal-review.md](signal-review.md) for units, corrections and unknown-state boundaries.
