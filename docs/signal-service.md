...
观测发布使用 `observations.jsonl.lock`，与入场邮件最终核验及 SMTP 调用互斥。邮件传输较慢时可能推迟扫描写盘，但不持有行情刷新锁；扫描子进程仍受配置的超时预算约束。升级时同时停止并更新扫描 writer 和邮件消费者，不能让旧 writer 绕过发布锁；离线备份或清理前确认两者均已停止，运行中不删除锁文件。
