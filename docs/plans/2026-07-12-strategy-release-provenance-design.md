...
- the promotion gate queries or otherwise verifies the live SCM review record, proves that its reviewer is independent from the candidate/release author, checks the exact candidate fingerprint and implementation commit, then captures its coordinates, canonical snapshot, and hash;
