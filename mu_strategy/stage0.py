...
class Stage0CycleRecorder:
    ...
    def observe_symbol(
        ...
        *,
        ...
        if data_error is not None:
            if trusted is None or trusted.allowed:
                trusted = _blocked_reference(
                    ...
                    reason=_health_reason_for_data_error(data_error),
                )
            failure_code = _failure_code_for_data_error(data_error, bundle=bundle)
            ...
            return self._record(Stage0SymbolResult(None, data_error, observation))
        ...
        return self._record(Stage0SymbolResult(result, None, observation))

    def commit(self) -> None:
        try:
            self._repository.append_cycle(
                Stage0ObservationCycle(
    ...
    def _record(self, result: Stage0SymbolResult) -> Stage0SymbolResult:
    ...
    def _build_observation(
        ...
        *,
        ...
        observed_at_ms = (
            ...
            if bundle is not None and bundle.observed_at_ms is not None
            else self.created_at_ms
        )
        provenance = (
            "canonical_trusted_generation"
            if bundle is not None and bundle.load_context is not None
...
def _health_reason_for_data_error(data_error: dict[str, Any]) -> HealthReason:
    value = data_error.get("status_reason")
    if isinstance(value, str):
        try:
            return HealthReason(value)
        except ValueError:
            pass
    if data_error.get("reason") == "trusted_provenance_incomplete":
        return HealthReason.MALFORMED_MANIFEST
    return HealthReason.CACHE_READ_FAILED


def _failure_code_for_data_error(
    data_error: dict[str, Any],
    *,
    bundle: CandleBundle | None,
) -> ObservationFailureCode:
    if bundle is None:
        return ObservationFailureCode.TRUSTED_DATA_LOAD_FAILED
    if data_error.get("reason") == "trusted_provenance_incomplete":
        return ObservationFailureCode.TRUSTED_PROVENANCE_INCOMPLETE
    return ObservationFailureCode.TRUSTED_DATA_BLOCKED
