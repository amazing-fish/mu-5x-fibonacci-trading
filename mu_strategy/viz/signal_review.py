...
def render_signal_review(report: dict, *, live=False) -> str:
    ...
    scan_note = "展开可核对每次扫描。"
    if scans.get("display_truncated"):
        scan_note = f'明细只保留最后 {scans.get("display_limit")} 条观察；上方统计仍覆盖完整窗口，筛选仅作用于这些明细。'
...
<p class="secondary">相邻同状态的正常等待已合并，其余逐条显示。{_e(scan_note)}</p><p id="scan-count" class="result-count" aria-live="polite">加载了 {len(scan_records)} 条明细</p>
