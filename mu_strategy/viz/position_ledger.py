...
def render_position_cards(view, *, editable=False):
    ...
    for position in sorted(view["positions"], key=lambda item: item["status"] != "open"):
        ...
        cards.append(f'''<article class="position-card" id="position-{identity}">
          ...
          <p class="secondary">最近成交时手记：stage {_e(position['recorded_stage'] or '未知')} · 止损 {_e(position['recorded_stop_price'] or '未知')}</p>
          <details id="fills-{identity}"><summary>成交明细 · {len(position['fills'])} 笔</summary><div class="table-wrap"><table>
          ...
          <details class="evidence" id="position-source-{identity}"><summary>记录来源与更正历史</summary>
            ...
            <div class="table-wrap"><table><thead><tr><th>录入时间 / 北京时间</th><th>成交编号 / 版本</th><th>记录值</th><th>实际成交时间</th><th>备注 / 更正原因</th></tr></thead><tbody>{history}</tbody></table></div></details></article>''')
    return ''.join(cards)
...
def render_position_editor(view, *, stylesheet, position_id=None, fill_id=None, source=None, draft=None, error=None, saved=False):
    ...
    notice = f'<p class="notice" role="alert">{_e(error)} 输入仍保留，可修改后重新提交。</p>' if error else '<p class="save-result" role="status">已保存实际成交记录。</p>' if saved else ''
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
      ...
      <header class="masthead"><div class="brand">MU<span>人工成交台账</span></div><nav><a href="/">每日复盘</a><a href="#positions">已记录持仓</a><a href="#position-form">记录成交</a></nav></header>
