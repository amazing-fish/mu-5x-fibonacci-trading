...
- 每次 `refresh_market_data` 成功提交 `current.json` 后才执行 generation 回收：默认 `--keep-generations 3`，按已提交 generation 目录的修改时间保留最近 N 个，并无条件额外保护 `current.json` 指向的 generation；N 必须至少为 1。带有严格、自绑定 `retention-pin.json` 的 generation 也会被保护，用于保留 strategy-release provenance 引用的历史证据。consumer 在同一 publication/reclamation 锁内把 manifest 绑定的 CSV 复制为只读 load-context 快照，因此锁释放后的回收不会破坏已打开的读取上下文。pointer 替换后若父目录同步失败，本轮跳过删除；其他回收失败同样只记录 warning，不会把已发布的可信数据判为失败，也不会放宽任何 consumer data-integrity gate。`--dry-run` 仍正常发布本次 refresh，但只报告受保护项、回收候选及可释放字节，不删除旧 generation。
- `refresh_runs.jsonl` 和 `refresh_market_data` JSON 输出会暴露 per-symbol/per-interval diagnostics，包括 `refresh_segments`、最多 5 条 `slowest_segments`、失败/非 ok 的 `failed_segments`，以及按 symbol 汇总的 `blocking_symbols`。每次回收还记录候选/已删除 generation id、可释放/已释放字节和失败原因；run log 最多保留最近 1000 行，达到上限后原子截断最旧记录，避免长期 loop 单调增长。
