...
- 每次 `refresh_market_data` 成功提交 `current.json` 后才执行 generation 回收：默认 `--keep-generations 3`，按已提交 generation 目录的修改时间保留最近 N 个，并无条件额外保护 `current.json` 指向的 generation；N 必须至少为 1。带有严格、自绑定 `retention-pin.json` 的 generation 也会被保护，用于保留 strategy-release provenance 引用的历史证据。所有 refresh 都在显式 lifecycle context 内从选择 prior generation 到完成提交/回收持有独立的跨进程锁，异常退出也会释放，避免 direct use-case 或 CLI refresh 删除对方仍在增量复用的输入；consumer 则在 publication/reclamation 锁内建立只读 load context。显式 `open_context()` 快照整个 manifest；普通单-symbol load 只复制 query 的 effective dataset bytes，并为其他 manifest datasets 保留只读、可随 context 自动关闭且允许 Windows delete-sharing 的 file handles。复用同一 context 查询其他 key 时从该 lease 补齐，因此一次查询不会复制整个 universe，generation 被回收后仍可继续读取。Windows 的 `Global\\` mutex 使用显式 DACL，允许 SYSTEM、Administrators 和 Authenticated Users 下的 service、scheduled-task 与 interactive refresh 共享同一锁。pointer 替换后若父目录同步失败，本轮跳过删除；其他回收失败同样只记录 warning，不会把已发布的可信数据判为失败，也不会放宽任何 consumer data-integrity gate。`--dry-run` 仍正常发布本次 refresh，但只报告受保护项、回收候选及可释放字节，不删除旧 generation。
- `refresh_runs.jsonl`、canonical `RefreshRun.reclamation` 和 `refresh_market_data` JSON 输出会暴露 per-symbol/per-interval diagnostics，包括 `refresh_segments`、最多 5 条 `slowest_segments`、失败/非 ok 的 `failed_segments`，以及按 symbol 汇总的 `blocking_symbols`。每次回收还记录候选/已删除 generation id、可释放/已释放字节和失败原因；run log 最多保留最近 1000 行，达到上限后原子截断最旧记录，避免长期 loop 单调增长。若最后一条记录在 UTF-8 多字节字符中间撕裂，下一次追加会先按 bytes 丢弃该不完整尾记录，再严格解码并保留此前完整记录。
