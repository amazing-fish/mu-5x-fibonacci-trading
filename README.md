...
- 每次 `refresh_market_data` 成功提交 `current.json` 后才执行 generation 回收：默认 `--keep-generations 3`，保留最近 N 个 generation，并无条件额外保护 `current.json` 指向的 generation；N 必须至少为 1。回收失败只记录 warning，不会把已发布的可信数据判为失败，也不会放宽任何 consumer data-integrity gate。`--dry-run` 仍正常发布本次 refresh，但只报告回收候选及可释放字节，不删除旧 generation。
